    ...
    with torch.inference_mode():
        for i, data in enumerate(tqdm(testing_loader, 0)):
            ...
            image = torch.permute(torch.squeeze(
                normalize_tensor(sample)), (1, 2, 0)) * -1
            label = torch.permute(torch.squeeze(lensed), (1, 2, 0))
            odak.learn.tools.save_image(
                f"results_24-512/{i}output.jpeg", image, cmin=-1, cmax=1)

            odak.learn.tools.save_image(
                f"results_24-512/{i}groundtruth.jpeg", label, cmin=0, cmax=1)


if __name__ == "__main__":
    network = UNet(in_channels=3, out_channels=3)  # for rgb channels
    checkpoint = torch.load("diffusion_model_50_10_24-512t.pth")
