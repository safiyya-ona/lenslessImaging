...
class DiffuserCamDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            y = transform_propagated(np.load(propagated_image))
...
class DiffuserCam:
    def __init__(
        self, path, training=False, testing=False, training_short=False
    ) -> None:
        self.path = Path(path)
        ...
        if training:
            ...
            ) = self.get_dataset_images(self.path, "dataset_train.csv")
        ...
        if testing:
            ...
            ) = self.get_dataset_images(self.path, "dataset_test.csv")
        ...
        if training_short:
            (
                ...
                training_propagated,
                training_ground_truth,
            ) = self.get_dataset_images(self.path, "dataset_train_short.csv")
            self.train_dataset = DiffuserCamDataset(
                training_diffused, training_propagated, training_ground_truth
            )
    ...
    def get_dataset_images(self, path, filename):
        ...
        for label in tqdm(labels, desc="Loading images"):
            diffuser_image = (
                path / "diffuser_images" / label.replace(".jpg.tiff", ".npy")
            )
            ground_truth_image = (
                path / "ground_truth_lensed" / label.replace(".jpg.tiff", ".npy")
            )
            propagated_diffused_image = (
                path
                / "propagated_rgb_diffused_images"
                / label.replace(".jpg.tiff", ".npy")
            )
            if diffuser_image.exists() and ground_truth_image.exists():
                if not propagated_diffused_image.exists():
                    new_image = to_tensor(np.load(ground_truth_image)).to(DEVICE)
                    if simulator is None:
                        simulator = Simulation(resolution=new_image.shape[1:])
                    new_propagated_image = simulator.diffuse_rgb_image(new_image)
                    np.save(
                        propagated_diffused_image,
                    ...
                    try:
                        ...
                        if prop_image.shape != (3, 270, 480):
                            new_image = to_tensor(np.load(ground_truth_image)).to(
                                DEVICE
                            )
                            if simulator is None:
                                simulator = Simulation(resolution=new_image.shape[1:])
